...
var listadeComentarioscond = new Array();

const btnguardarcond = document.getElementById('btnguardarCondicionales');
const comentarioguardadocond = document.getElementById('ResultadoDComentariosCondicionales');
const cantDeComentarioscond = document.getElementById('cantDeComentariosCondicionales');

btnguardarcond.addEventListener('click', guardar)
...
function guardarEnPC() {
    if (!window.localStorage.key('ListaComments')) {
        window.localStorage.setItem('ListaComments', JSON.stringify(listadeComentarioscond));
    }
    else {
        window.localStorage.setItem('ListaComments', JSON.stringify(listadeComentarioscond));
    }
...
function obtenerEnPC() {
    if (window.localStorage.key('ListaComments')) {
        listadeComentarioscond = JSON.parse(window.localStorage.getItem('ListaComments'));
    }
}
...
function carga(e) {
    ...
    comentarios.Numero = listadeComentarioscond.length;
    ...
    if (typeof (comentarios.Nombre) != 'string' || comentarios.Nombre === '') {
        ...
        listadeComentarioscond.push(comentarios)
    }
}
 ...
 */
function cargaenDiv() {
    comentarioguardadocond.innerHTML = "";

    listadeComentarioscond.forEach((c, ind) => {
        ...
        cantDeComentarioscond.innerHTML = listadeComentarioscond.length;
        ...
        div.append(br)
        comentarioguardadocond.appendChild(div)
    })
}
...
function Borrar(e) {
    ...
    listadeComentarioscond.splice(parseInt(index), 1);
    listadeComentarioscond.forEach((b, ind) => {
        b.Numero = ind
    })
